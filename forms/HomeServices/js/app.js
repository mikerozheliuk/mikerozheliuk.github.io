(() => {
    ...
    da.init();
    const BLOG_ITEMS_SELECTOR = ".blog__items";
    const BLOG_JSON_URL = "files/blog.json";
    const MAX_BLOG_ITEMS = 3;
    const blogItems = document.querySelector(BLOG_ITEMS_SELECTOR);
    const viewMoreButton = document.querySelector(".blog__more");
    let allBlogItems = [];
    let displayedBlogItems = 0;
    if (blogItems && viewMoreButton) {
    ...
    async function loadBlogItems() {
        try {
            ...
            allBlogItems = responseResult.items;
            if (allBlogItems.length === 0) hideViewMoreButton(); else initBlog();
        } catch (error) {
            console.error("Error:", error.message);
        }
    }
    function initBlog() {
        displayedBlogItems = 0;
        showNextBlogItems();
    }
    function showNextBlogItems() {
        const remainingItems = allBlogItems.slice(displayedBlogItems, displayedBlogItems + MAX_BLOG_ITEMS);
        if (remainingItems.length === 0) {
            console.log("No more items to load.");
            hideViewMoreButton();
            return;
        }
        showBlogItems(remainingItems);
        displayedBlogItems += MAX_BLOG_ITEMS;
        if (displayedBlogItems >= allBlogItems.length) hideViewMoreButton();
    }
    function showBlogItems(items) {
        items.forEach((item => {
            buildBlogItem(item);
        }));
    }
    function buildBlogItem(item) {
        const blogItemTemplate = `\n    <article class="blog__item item-blog">\n      ${item.image ? `\n        <a href="${item.url}" class="item-blog__image-ibg">\n          <img src="${item.image}" alt="image" />\n        </a>\n      ` : ""}\n\n      <div class="item-blog__date">${item.date}</div>\n      <div class="item-blog__title">\n        <a href="${item.url}" class="item-blog__link-title">${item.title}</a>\n      </div>\n      <div class="item-blog__text">${item.text}</div>\n\n      ${item.tags ? `\n        <div class="item-blog__tags">\n          ${Object.entries(item.tags).map((([tag, url]) => `\n            <a href="${url}" class="item-blog__tag">${tag}</a>\n          `)).join("")}\n        </div>\n      ` : ""}\n    </article>\n  `;
